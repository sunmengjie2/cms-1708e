...
<form name="articleform"  id="articleform">
  ...
  <div class="form-group">
    ...
    <select class="form-control" id="channel" name="channelId" >
      ...
      <c:forEach items="${channels}" var="cat">
      		<option value="${cat.id}" ${cat.id==articles.channelId?"selected":"" }>${cat.name}</option>
      </c:forEach>
    </select>
  </div>
 ...
 <div class="form-group">
    ...
    <input type="hidden" name="picture" value="${articles.picture }">
    <img alt="" src="/pic/${articles.picture }">
	...
	function channelChange(){
		...
		$.post("/user/getCategoris",{cid:$("#channel").val()},
				function(data){
					$("#category").empty();
					for ( var i in data) {
						if(data[i].id=='${articles.categoryId}'){
		...
		console.log("222222222222")
		  ...
		  $.ajax({url:"updateArticles",
			  ...
			  success:function(data){
				  ...
				  //  
				  wordspace($("#postLink"),"/user/articles")
			  }
		 })
		}
